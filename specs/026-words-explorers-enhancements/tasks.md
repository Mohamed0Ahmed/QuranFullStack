...
- [x] T006 [US1] Add the search predicate to `BaseRowsSql` in `BE-INF/Persistence/Reads/Quran/Words/WordTypes/EfWordTypesReader.Sql.cs`: parameterized `ILIKE @searchPattern` (`%normalized%` as a VALUE) against `quran_words_unique_tashkeel.search_text_normalized` joined/EXISTS on the base's word id; normalize via `ArabicSearchQueryNormalizer` (T002). Plumb the param through `EfWordTypesReader.cs` `GetRowsAsync`/`GetTableRowsAsync` and the reader interface `BE-ABS/Quran/Words/WordTypes/IWordTypesReader.cs`. Grouped detail reads (`.GroupedDetails.*`) take NO search param.
...
- [x] T050 [P] [US8] New DTO `BE-ABS/Quran/Words/WordTypes/Responses/WordTypeScopeCountsDto.cs` (`WordsCount, RootsCount, StemsCount, LemmasCount` ints) + reader method signature on `BE-ABS/Quran/Words/WordTypes/IWordTypesReader.cs`.
- [x] T051 [US8] New query/handler `BE-APP/Quran/Words/WordTypes/Queries/GetWordTypeScopeCounts/` (query record = full scope incl. search + flags; outcomes `Success | InvalidFilter`; validation via `WordTypesHandlerValidation`; `hasSearch`-only logging).
...
- [x] T053 [US8] Caching: `WordTypesCacheKeys.ScopeCounts(...)` with EVERY scope input + `CachedWordTypesReader.cs` wrap; entry options mirror the table read (`WordTypesCacheEntryOptions.cs`).
- [x] T054 [US8] Controller action `GET api/words/word-types/scope-counts` in `BE-API/Controllers/Words/WordTypesController.cs` + Arabic messages in `BE-API/Common/ApiMessages.cs`; zero-row valid scope → 200 all-zeros; invalid scope → 400.
- [x] T055 [US8] Backend tests in `BE-TST/Quran/WordsWordTypes/`: EQUALITY MATRIX — for scopes covering each main type, a child, case/tense/voice variants, ±search, ±has-flags: each count equals the corresponding tableView's `TotalCount`; single-SQL-command budget pinned via the `SqlCommandCountInterceptor` pattern (`BE-TST/Quran/Words/SqlCommandCountInterceptor.cs`); cache-key isolation per scope input; count-family audit (no `words_count`-backed values anywhere in the read).

### Implementation — frontend

- [x] T056 [US8] Regenerate the frontend API client (`FE/core/api/generated/`) per the repo api-contract staleness guard so `WordTypeScopeCountsDto` is generated; re-export per the words models convention. (NOTE: regen also swept in P1–P3 backend changes the committed generated contract was stale against — has-flags params, page-size default docs — since generated files must match code and hand-editing them is forbidden. Flagged for review.)
- [x] T057 [US8] `WORDS/data-access/word-types.api.ts`: `getScopeCounts(scope)`; `WORDS/state/word-types-explorer.facade.ts`: load counts on scope change ONLY (type/childCode/case/tense/voice/search/flags — NOT tableView, NOT page); `WORDS/state/word-types-cache.ts`: scope-counts key mirroring backend components; update the three spec files (incl. "tabs/page changes trigger no counts fetch").
- [x] T058 [US8] New component `WORDS/components/word-type-scope-counts/` (`.ts/.html/.scss/.spec.ts`): four labeled counts reusing the existing view tabs' SHORT labels verbatim — RTL order كلمات | جذور | أصول | صيغ (tabs NOT renamed; spec Clarifications; labels in `word-types.labels.ts`, TDZ getters); non-interactive; states — loading skeleton / compact error + "إعادة المحاولة" retry (refetches counts only) / zeros. Place in `word-types-explorer-page.component.html` BETWEEN the filter strip and the table-view tabs; mounted-shell invariant preserved; counts failure never blocks the table; table failure hides the strip numbers. (Strip is **presentational**: the page passes scope-counts `state` in and handles the `retryRequested` output via the facade — page → facade → presentational-child boundary. The page absorbing this state ownership nudges it just over the 400-line review threshold; splitting the page further is deferred and tracked as review debt, since the boundary matters more than the line count.)
- [x] T059 [US8] Update words README (strip, URL/cache identity, states) + reads README (new read, scoped family, 1-command budget).
- [x] T060 [US8] Perf gate PASS (evidence: `Backend/report/feature-026-words-explorers-enhancements/002-plan-p4-perf-gate.md`): uncached `/scope-counts` single-command timing on full DB — `type=noun` p95 241.5 ms, `type=verb` p95 106.5 ms (budget p95 ≤ 2s); 1 SQL command pinned by `ScopeCounts_UsesSingleSqlCommand`. Deploy-smoke PASS (live API, launch profile): health 200; noun/verb 200 with live FR-016 equality vs all four tableView totals (incl. search+flag composition); zero-row → 200 all-zeros; invalid type / 65-char search → 400; worst live uncached call 0.821 s. Plan-P4 checkpoint: backend 1271 + frontend 1113 green; committed.
