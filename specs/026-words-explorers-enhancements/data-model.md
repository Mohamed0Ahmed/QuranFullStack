...
|---|---|---|---|
| `search` | `string?` | trim; empty→null; defensive max length (~64) else `InvalidFilter`; never logged (only `hasSearch`) | normalized-contains match on word identity text (`text_imlaei_simple`); part of the shared scope — words view, grouped views, and scope counts all inherit it |
