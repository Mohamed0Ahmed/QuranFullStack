...
public sealed class AyahStudyCorruptCoveredAyahKeysTests
{
    private const string VerseKey = "114:7";
    private const string SourceKey = "ar-muyassar";

    [Fact]
    public void GetAyahStudy_corrupt_covered_ayah_keys_returns_empty_list_and_logs_warning_naming_ayah_and_source()
    {
        ...
        var coveredAyahKeys = MushafReaderValueMapper.ParseCoveredAyahKeys(
            "{\"not\":\"an array\"}",
            VerseKey,
            SourceKey,
            logger);

        coveredAyahKeys.Should().BeEmpty(
