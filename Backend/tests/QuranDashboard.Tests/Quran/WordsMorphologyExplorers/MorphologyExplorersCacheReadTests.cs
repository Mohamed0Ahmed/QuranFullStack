...
public sealed class MorphologyExplorersCacheReadTests(MorphologyExplorersTestFixture fixture)
{
    ...
    public async Task Lemma_catalogue_reuses_summary_all_cache_across_sort_and_page_changes()
    {
        ...
        interceptor.Reset();
        await reader.GetLemmasPageAsync("كلم", LemmaSort.Occurrences, LemmasCountFilter.None, 2, 10, CancellationToken.None);
        await reader.GetLemmasPageAsync(null, LemmaSort.Alpha, LemmasCountFilter.None, 3, 5, CancellationToken.None);
    ...
    public async Task Stem_catalogue_reuses_summary_all_cache_across_sort_and_page_changes()
    {
        ...
        await reader.GetStemsPageAsync(null, StemSort.MushafOrder, StemsCountFilter.None, 1, 50, CancellationToken.None);
        interceptor.CommandCount.Should().BeGreaterThan(0);

        interceptor.Reset();
        await reader.GetStemsPageAsync("علم", StemSort.Occurrences, StemsCountFilter.None, 2, 10, CancellationToken.None);
        await reader.GetStemsPageAsync(null, StemSort.Alpha, StemsCountFilter.None, 3, 5, CancellationToken.None);
