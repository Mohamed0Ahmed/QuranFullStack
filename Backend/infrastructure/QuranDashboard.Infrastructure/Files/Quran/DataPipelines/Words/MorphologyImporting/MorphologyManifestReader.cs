...
public sealed class MorphologyManifestReader
{
    ...
    public async Task<MorphologyManifest> ReadAsync(string sourceRoot, CancellationToken ct)
    {
        ...
        var manifest = document.RootElement.Deserialize<MorphologyManifestDocument>(JsonOptions)
        ...
        var files = new Dictionary<string, MorphologyManifestFile>(StringComparer.Ordinal);
        foreach (var file in manifest.Files)
        {
            ...
            {
                ValidateRecordCount(fullPath, file.ExpectedRecordCount.Value, ct);
            }

            files.Add(localPath, file with { FullPath = fullPath });
        }

        return new MorphologyManifest(files);
    }

    public async Task<MorphologyFileDigests> CaptureDigestsAsync(
        string sourceRoot, CancellationToken ct)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(sourceRoot);

        var digests = new Dictionary<string, FileDigest>(StringComparer.Ordinal);
        foreach (var relativePath in AllowedSourceFiles)
        {
            ...
            digests[relativePath] = new FileDigest(fileInfo.Length, sha256);
        }

        return new MorphologyFileDigests(digests);
    }

    public async Task<bool> VerifyDigestsUnchangedAsync(
        string sourceRoot, MorphologyFileDigests before, CancellationToken ct)
    {
        var after = await CaptureDigestsAsync(sourceRoot, ct);
        return before.Equals(after);
    }

    private static void ValidateManifestFileSet(IReadOnlyList<MorphologyManifestFile> files)
    {
...
public sealed record MorphologyManifest(
    IReadOnlyDictionary<string, MorphologyManifestFile> Files);

public sealed record MorphologyManifestFile(
    string Role,
    string Path,
    string? OriginPath,
...
public sealed class MorphologyFileDigests(IReadOnlyDictionary<string, FileDigest> digests)
{
    public IReadOnlyDictionary<string, FileDigest> Digests { get; } = digests;

    public bool Equals(MorphologyFileDigests? other)
    {
...
internal sealed record MorphologyManifestDocument(
    List<MorphologyManifestFile> Files);
