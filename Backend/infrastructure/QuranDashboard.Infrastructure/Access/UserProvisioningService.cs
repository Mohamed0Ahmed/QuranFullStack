    ...
    IOptions<OwnerBootstrapOptions> bootstrapOptions) : IUserProvisioningService
{
    ...
    private async Task<ProvisionedUser> ReconcileExistingAsync(User existing, string logtoSub, CancellationToken ct)
    {
        ...
        // Never auto-revive or auto-promote a Disabled user on login.
        ...
        // Only promote a pre-existing owner-email user when their email is IdP-verified.
        var profile = await profileSource.GetProfileAsync(logtoSub, ct);
    ...
    {
        // Owner-email first login gets Owner/Active only when the IdP reports the email verified; otherwise Pending/no-role.
        ...
        try
        {
            ...
            {
                // A negative role may have been cached before the row existed; drop it.
                roleResolver.Evict(logtoSub);
            }
        ...
        {
            // Concurrent first login won the unique-index race: detach our insert, return the row that landed first.
            ...
            if (winner is null)
            {
                // No row under our sub => the conflict is the email unique index (Logto subject recreated
                // with a new sub but an already-registered email): an expected business conflict, not a fault.
