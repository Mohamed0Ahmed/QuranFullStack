...
{
    private sealed record SourceAyahSnapshot(
        int AyahId,
        int OrderValue,
    ...
    private sealed record SourceSnapshot(
        long UnitId,
        bool IsGrouped,
        IReadOnlyList<SourceAyahSnapshot> Ayahs)
    {
        public static async Task<IReadOnlyDictionary<long, SourceSnapshot>> LoadAsync(
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (unitIds.Count == 0)
            {
                return new Dictionary<long, SourceSnapshot>();
            }
            ...
            var ayahsByUnit = ayahs
                ...
                .ToDictionary(
                    group => group.Key,
                    group => (IReadOnlyList<SourceAyahSnapshot>)group
                        .Select(ayah => new SourceAyahSnapshot(
                            ayah.AyahId,
                            ayah.OrderValue,
