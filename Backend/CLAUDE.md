...
first. Only the C#-specific detail lives here.

- **Scope:** `.cs` under `api/`, `application/`, `domain/`, `infrastructure/`. Not `tests/`, not
  `tools/` (the DataImporter), not `scripts/`, and never generated migration files
  (`Migrations/*.Designer.cs`, `*ModelSnapshot.cs`).
- **No `///` XML-doc anywhere in scope** — no Swagger or API-doc consumer reads it, and it is the
  single largest source of signature-paraphrasing noise in this codebase. That includes
  controllers, endpoints, handlers, DTOs, entities, and value objects.
- `#pragma warning disable`, `// <auto-generated />` and similar directives are not comments for
  this rule. Never remove them.
- Layer boundaries, invariants, and contract behaviour belong in the nearest `README.md` — the
  Persistence, DataPipelines and Controllers READMEs already carry that weight; extend them
