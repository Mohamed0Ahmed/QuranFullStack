...
description: >-
  Action skill for cleaning and consolidating C# global usings in the Quran
  Dashboard .NET backend projects and tests. Use when the user wants to clean up,
  dedupe, tidy, or reduce repeated using directives; promote repeated layer-safe
  namespaces into GlobalUsings.cs; remove redundant local usings; or fix sprawling,
  missing, or shared GlobalUsings.cs files. It promotes only common, layer-safe,
  non-feature-specific namespaces repeated in more than five files in the same
  project, preserves Clean Architecture boundaries from BACKEND_STRUCTURE.md,
  verifies with dotnet build, and never edits BACKEND_STRUCTURE.md. Not for one
  missing using, TypeScript imports, or C# using resource/disposal statements.
---
