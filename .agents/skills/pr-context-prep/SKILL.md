...
description: >-
  ...
  monorepo diff/status against the base branch, classifies changed paths (Backend
  importer, Backend read API, Frontend Angular, specs/docs, or cross-stack), and
  ...
  merge-readiness call. Review/prep only: never edits code, commits, or opens the PR.
  Use that Claude skill as the single source of truth.
---
...
For staging, commit planning, and push readiness, use `commit-workflow`.
