...
description: >-
  ...
  (.NET backend + Angular frontend). Use when the user asks to review code, a diff,
  PR, branch, or change; asks whether work is ready to merge, follows coding
  principles, or respects architecture; or shares freshly implemented backend or
  frontend code for a quality check. Also use for Spec Kit implementation reviews
  when requests mention phases, user stories, task IDs like T013-T018, spec/plan/
  tasks/contracts/quickstart artifacts, or specs feature folders. Produces findings,
  risks, recommendations, and a verdict only; do not implement fixes unless
  explicitly asked.
---
...
- `references/clean-code-guard/naming-and-functions.md`
- `references/clean-code-guard/comments-and-formatting.md`
- `references/clean-code-guard/solid.md`
- `references/clean-code-guard/dry-kiss-yagni.md`
- `references/clean-code-guard/ai-failure-modes.md`
- `references/clean-code-guard/review-checklist.md` — structured walk-through for a deep code-quality review.
- `references/clean-code-guard/sources.md` — bibliography; read only when a finding is contested and you need the source citation.
...
- Installed `$test-guard` skill — the nine test-code rules.
- `$test-guard` reference `references/dotnet.md` — for backend tests (xUnit,
  `WebApplicationFactory`, EF Core + PostgreSQL via Testcontainers, `ApiResponse`).
- `$test-guard` reference `references/jest.md` — for frontend Angular/TypeScript tests.
...
- `SPEC_KIT_IMPLEMENTATION_REVIEW.md` — the extra
