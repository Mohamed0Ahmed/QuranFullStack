...
8. **Exact files expected to be created, modified, or deleted.** Relative to `Frontend/quran-dashboard-ui/`:
   ...
   **Create:** none as planned. The "Phase 11 manifest amendment — 2026-08-10" below authorizes thirteen creates that this list did not anticipate, all of them §1.2 responsibility splits forced by review findings; they are named there.
...
**Phase 11 manifest amendment — 2026-08-10.** Owner-authorized under §0 ("amend this plan before expanding a manifest") and §12, after the first formal review cycle, so that task 10.9's "separate implementation fixes the selected findings" has a manifest to work inside. The occasion is ER-1: the four Words Compact/Medium detail drawers still declared a **fifth** dialog geometry (`.qd-modal.explorer-detail-modal`, `min(100%, 42rem)` wide, `min(88dvh, 42rem)` tall below Wide) with their own padding and body scroller, which left item 16 unmet for D48 and D49 and matched §12's "a fifth modal width" stop condition. The permanent visual authority (`.architecture/golden-ui/04 IMPLEMENTATION_HANDOFF_SUMMARY.md` §2, decision 8) kills the five-geometry drawer family outright, so a recorded permanent exception is not available and the drawers are migrated instead of retained. Files added to the Phase 11 **Modify** manifest, relative to `Frontend/quran-dashboard-ui/`:

- `src/app/features/words/components/{root,lemma,stem,word-type}-details-panel/*.component.{html,ts}` — the modal branch composes `qd-modal-shell variant="overlay"` with `[showTitle]="false"`, `[showClose]="false"`, `[hasFooter]="false"`, `[flushBody]="true"` and `[trapFocus]="drawerTrapEnabled()"`. The `.ts` edit is exactly the `imports` array (`A11yModule` and `ModalScrollLockDirective` out, `QdModalShellComponent` in) plus deletion of `onBackdropClick`, which the shell now owns. Every `data-testid` is preserved through `testIdPrefix` / `dialogTestId`.
- `src/app/features/words/components/{lemma,stem}-details-panel/*.component.spec.ts` — the drawer's accessible name moves from `aria-label` to the shell's `aria-labelledby`; the two specs that asserted the old attribute now assert the referenced element's text. No assertion was dropped.
- `e2e/abwab-slice-j-widths.e2e.ts` — the "42rem hold-out" probe becomes the named-`overlay` probe: `data-qd-modal-variant="overlay"` and a computed `736px`.

`src/styles/_components.scss` and the `styles` / `words` READMEs were already in the manifest: the geometry blocks and the then-zero-consumer `.qd-modal` / `.qd-modal-backdrop` pair were deleted there under §7.1, and `.explorer-detail-modal` survives only as a content class declaring no inline size, no block size and no scroller.

The same authorization covers the files ER-2 through ER-8 touch. Most already sit inside the union of the Phase 1–10 manifests; **thirteen are genuinely new and existed in no manifest, so this amendment is what authorizes their creation** — §1.2 requires an amendment before a new file, and each is a responsibility split a finding forced rather than a new abstraction. They are marked *(new)* below, and no file in the final diff is left unaccounted for under item 18:

- **ER-2** — `src/app/shared/ui/data-table/data-table.component.{html,ts,spec.ts}`: `qd-data-table` becomes the one real consumer of `qd-refreshing-indicator` and `.qd-refreshing-region`. The other nominated-but-unconsumed owners (`.qd-selected-thread`, `.qd-surface--sunken`/`--chrome`, `.qd-card--feature`, `.qd-page-header--split`/`__eyebrow`/`__meta`/`__actions`, `.qd-toolbar--workspace`, `.qd-count-chip`, `.qd-floating-layer__item--danger`) are deleted inside `_components.scss` / `_layout.scss`, with the actual owner named in the styles/shared READMEs — all four files already in the manifest.
- **ER-3** — the three Words explorer pages, the three `*-detail-overlay-adapter` components, `word-type-table-view-tabs`, `word-types-explorer-page` (the `role="tabpanel"` the table view tabs now name), and `src/styles/_words-explorer-layout.scss`.
- **ER-4** — `access-admin-page.component.{html,ts,scss}`, `models/access-admin.models.{ts,spec.ts}`, `shared/ui/floating-layer/{floating-layer.directive.ts,floating-layer-placement.ts}`, and *(new)* the two `components/access-{account-permissions,owner-reconciliation}/` component sets (`.component.{html,ts,scss,spec.ts}` each) plus *(new)* `shared/ui/floating-layer/floating-layer-{keyboard,cursor,focus}.ts` and `floating-layer-keyboard.spec.ts`.
- **ER-5** — closed as a *class*, by sweeping every `[title]` in `src/` rather than by working the reported list. Nine surfaces had `title` as their only disclosure and now wrap: `access-audit-log`, `access-admin-page`'s context bar, `access-owner-reconciliation`, `access-user-picker`, `abwab-{relations,sections}-modal`, `abwab-side-panel`, `abwab-template-tree` and `abwab-templates-page`'s editor heading — each `.component.{html,scss,spec.ts}` as it applies. The seventeen remaining `[title]` sites were each resolved against their real focusable owner and left alone; the disposition and the sweep rule live in `.architecture/UI_STYLE_SYSTEM.md` §17.
- **ER-6 / ER-7** — `scripts/check-golden-ui-contract.mjs`, `.architecture/FRONTEND_STRUCTURE.md` and `FRONTEND_UI_RULES.md` (already in the manifest), plus *(new)* `features/access-admin/state/access-audit.store.spec.ts`.

Three outcomes of this amendment are recorded as deliberate rather than defects:

- **The migrated drawer keeps the panel's own header, so the shell's title is `qd-sr-only`.** `qd-details-workspace` already renders the panel label and the close button; showing the shell's title too would be a second visible heading. `[showTitle]="false"` keeps the `<h2>` mounted as the `aria-labelledby` target, which is what replaces the deleted `aria-label` — the accessible name is unchanged, and the bare header contributes zero geometry.
- **`access-admin-page.component.ts` lands at 376 lines** — over the §1.2 300-line soft limit, under the 400 hard limit, and down from 389. ER-4's two hard-limit breaches are closed (template 418 → 297, directive 411 → 281). Reaching the soft limit would require lifting the route-leave promise gate, the `matchMedia` Wide tracking (duplicated in nine other files, so a shared helper is the honest fix) or the audit search wiring — none of which the finding names, and the first is authorization-adjacent dirty-state protection. Deferred rather than broadened here.
- **`word-type-table-view-tabs` is the one migrated tablist whose panel is not its own child.** It controls the words table in a sibling grid column, so `word-types-explorer-page` supplies the `id`/`role="tabpanel"`/`aria-labelledby` and the component takes a `panelId` input; the id is absent while no tabs render, so no fake panel exists.
