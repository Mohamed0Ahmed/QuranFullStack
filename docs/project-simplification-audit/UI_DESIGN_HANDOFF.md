...
| F05 | Button and action group | `qd-btn` variants, icon actions, toolbar actions, lifecycle actions, row actions. | 44px mobile target, label/icon/busy/disabled/focus, danger semantics. | Destructive versus safe; icon-only needs accessible name. | Heights, local copies, active translation, green used for generic hover, small controls. |
