...
| L6 | **Converge on one shared details panel shell** for the genuinely duplicated Words details structures. The shell owns header structure, tabs placement, the stable content container, inline/modal/frameless behavior, ARIA/id wiring, and shared loading-geometry behavior. Labels, tab keys, disabled rules, content, empty/error wording and entity-specific views stay page-specific. **Do not** absorb the page-level domain `@switch` into a domain-aware shared component. **Amended after Phase 15 — recorded here rather than taken silently:** "disabled rules" stay entity-specific *where they depend on entity state*; a predicate written purely over shell-owned state belongs to the shell. All four panels' predicates were byte-identical and read only `emptySelection()`, `notFound()` and `view()` — they encode no view-key semantics and no domain vocabulary, so they do not engage the concern this decision protects (which its own next sentence names). Retaining four copies would have preserved exactly the duplication X-5 targets, in the one phase whose purpose is removing it. No override input was added: no entity has a second policy, Word Types' kind-dependence is handled by filtering the *tab set* rather than the predicate, and a function-valued input with zero consumers would be YAGNI. Cut that seam when a second policy actually appears. |
...
- Each entity keeps: its view key list, labels and aria strings, ~~its disabled predicate~~ (see the
  L6 amendment — the predicate proved to be pure shell state and moved into the shell), its
