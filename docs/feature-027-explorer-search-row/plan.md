...
- **Popover placement (correction):** the field lives inside `.uw-toolbar-recess` (a bordered, padded `--qd-surface-recessed` surface — `_words-explorer-layout.scss:14`) within a flex row. That recess uses **`overflow: visible`** (it sets no `overflow`), so an in-DOM `position: absolute` panel is **not** clipped — it renders **above the recess border via `z-index` / a floating stacking context** (no CDK Overlay, no body-mounted portal). Anchor directly **under the field**, mirror correctly in **RTL** (leading edge = right), and compute a **viewport-aware `max-height`**; when there is too little room below the field (near the viewport bottom / with a mobile keyboard), **flip the panel above it** rather than overflowing the viewport. Reuse **only** surah-jump-picker's `getBoundingClientRect()` max-height computation (`window.innerHeight − rect.bottom − padding`, clamped — `surah-jump-picker.component.ts:219–236`) and its `window:scroll/resize` reposition mechanics (`:156–162`), anchored to the **field** rather than a button trigger. **Stop condition:** the no-portal approach holds only while no ancestor clips with `overflow` — if one ever does, `z-index` cannot escape `overflow` clipping and a body-mounted overlay/portal becomes required.
...
- **Popover not clipped:** `.uw-toolbar-recess` uses `overflow: visible`, so verify the association popover renders above its border via stacking context (not clipped); anchors under the field; mirrors in RTL; is height-limited via the reused `getBoundingClientRect()` max-height; and **flips above the field** when it sits near the viewport bottom (reposition on `window:scroll/resize`).
