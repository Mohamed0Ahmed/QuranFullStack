...
| I2 | **The conditional-GET contract of the three reads** — match → `304` bodiless with `ETag` + `Cache-Control`, mismatch → `200` + fresh `ETag`, malformed and `*` → fail-open `200`, `404` with no validator headers, and the `304` path running zero queries **for the two list reads** (the template detail answers existence first by design — see `API_GUIDELINES.md`, conditional reads). The template detail's `404`-with-crafted-validator case is **PAID** by `Backend/tests/QuranDashboard.Tests/Smoke/SmokeAbwabTemplateReadTests.cs`; the rest stays open. All three routes are already catalogued, so these are additional dispatched cases, not new entries. This row's "next change to any conditional read" trigger has fired once (the template-detail ordering fix) and was paid exactly that far | `Tests/Smoke/` | **Acceptance criterion of the auth feature** (the standing due-date for every abwab smoke row above), **or** the next change to any conditional read — whichever comes first |
...
replacement rule, and every deletion had named replacement coverage. Three residues survived that
rule with no assertion anywhere, so they are recorded here rather than left as folklore. None of
them is a reason to hold the prerequisite; all three are cheap once someone is already in the file.
...
| TR2 | **`ACCESS_ME_CONTRACT_FIXTURES` has zero importers.** Phase 7 deleted `auth.testing.spec.ts`, which compared the fixture module to literals declared in that same module. `auth.testing.ts` itself was kept unchanged and is still compile-checked through its `AccessMeContractFixtures` annotation, and `provideAuthTesting()` still has composition consumers — but the four `/me` fixture payloads are now imported by nothing. Until they acquire a production consumer, a wrong field in them fails no test. **The test must assert** the pending / read-only / exact-permission / Owner responses against a real store, guard, or component decision, not against literals | `Frontend/quran-dashboard-ui/src/app/core/auth/auth.testing.ts` | **Authorization Phase 3**, which is what gives those fixtures their first real consumer. This row is an acceptance criterion of that phase, not open-ended debt |
| TR3 | **A frozen route count lives in a comment that nothing asserts, and it has already drifted.** `Smoke/Data/SmokeDataFixture.cs:12` justifies the exclusive canonical server with "seeding the database it reads would invalidate all 48 of them", referring to the pipeline sweep's per-route cases. The sweep is one theory over `SmokeRouteCatalog.Routes.Where(route => !route.ParityOnly)`, which is **49** rows today — the 2026-08-06 broad run executed 49 `SmokeRoutePipelineTests` cases. The number predates this prerequisite and is not load-bearing for the fixture's behavior, so nothing broke; but a count with nothing computing it is a rumour. **The test must assert** the sweep's case count against `SmokeRouteCatalog`'s own non-`ParityOnly` count, or the sentence must drop the number | `Backend/tests/QuranDashboard.Tests/Smoke/Data/SmokeDataFixture.cs`, `Smoke/SmokeRoutePipelineTests.cs`, `Smoke/SmokeRouteCatalog.cs` | The next route added to or removed from `SmokeRouteCatalog` — whoever adds the parity entry is already holding the count |
