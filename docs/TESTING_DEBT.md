...
| J2 | **The sections modal's delete flow still has no confirmation at all** — a section delete is one click, and the only thing standing between a misclick and a retired section is the backend's `409` when live doors remain. An archived-only section deletes silently. `qd-confirm-dialog` now exists to make this cheap | `features/abwab/components/abwab-sections-modal/` | The archive-confirmation slice, which was the non-goal this feature carved out; it should consume the same primitive for both |
| J3 | **The templates page's two hand-rolled `role="alertdialog"` confirms** — they predate the primitive and were deliberately left alone (non-goal). They are the last hand-written confirms in the app, and §17 now says not to write more | `pages/abwab-templates-page/abwab-templates-page.component.html` | The same slice as J2 — one retrofit pass, not two |
