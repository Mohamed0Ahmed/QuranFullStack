...
| AC3 | **The audit action-type list is a second hand-maintained cross-language mirror, and nothing fails when it drifts.** `ACCESS_AUDIT_ACTION_TYPES` (`models/access-admin.models.ts`) restates the ten members of `AccessAuditActionType`, whose C# names are the wire values `ListAccessAuditEventsHandler` accepts. They agree today, and drift degrades gracefully in both directions — an unaccepted filter value answers `400` rather than throwing, and an unmodelled type reaching a row stays legible because `ACCESS_ADMIN_LABELS.auditActionType` returns an unrecognised value unchanged — which is exactly why drift is silent: a backend enum member added or renamed simply stops being offered in the dropdown, and no test anywhere notices. This is the **same class of debt as row AC1** — a hand-maintained TypeScript copy of a backend enumeration — and it must be paid the same way rather than duplicated: see AC1 for the `core/auth/permission-code.ts` half, which is the larger of the two and has no spec file at all. **The test must assert** both directions of set equality between the declared TypeScript list and `Enum.GetNames<AccessAuditActionType>()`, modelled on `SmokeCoverageParityTests`, so that adding or retiring an action type is a deliberate two-sided change and never a silent one. The generated OpenAPI cannot carry this — `AccessAuditEventItem.actionType` and the filter parameter are both plain `string`, so nothing generated pins the membership set | The gate must live beside `Frontend/quran-dashboard-ui/scripts/check-permission-catalogue.mjs` (run by `npm run check:permission-catalogue`, the repo's only existing cross-language enumeration gate), reading `Backend/domain/QuranDashboard.Domain/Access/AccessAuditActionType.cs` and `Frontend/quran-dashboard-ui/src/app/features/access-admin/models/access-admin.models.ts` | The catalogue-deduplication work that retires the hand-maintained allowlist in `core/auth/permission-code.ts` — it already has to build the backend→frontend enumeration bridge and harden that same gate script, at which point a second enumeration is one more comparison rather than new machinery. **Or**, sooner: the next member added to or removed from `AccessAuditActionType` |
