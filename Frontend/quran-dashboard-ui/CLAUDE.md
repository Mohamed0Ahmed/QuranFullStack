...
| Any UI-visible change (tokens, layout, components, templates, styles) | `Frontend/quran-dashboard-ui/FRONTEND_UI_RULES.md` — the short mandatory rule set (ownership ladder, light-only scope, approved fonts, protected Quran boundary, responsive bands, one gutter, prohibited effects, `qd-state` no-growth). The permanent visual authority is `Frontend/quran-dashboard-ui/.architecture/golden-ui/`. |
