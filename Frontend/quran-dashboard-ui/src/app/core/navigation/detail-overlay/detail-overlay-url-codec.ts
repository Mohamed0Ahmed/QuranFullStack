...
// Fail closed on input; always serialize defaults so a future default change can't re-interpret an old shared URL.
...
// encodeURIComponent leaves `~` unescaped: escape it so the separator stays unambiguous; escape a literal `-` so it can't be misread as the null sentinel.
...
// Above Number.MAX_SAFE_INTEGER a digit run rounds to a different integer, silently resolving to another
// entity — the safe-integer guard keeps ids exact.
