...
for (const theme of ['light', 'dark'] as const) {
  for (const viewport of VIEWPORTS) {
    test(`--wide measures 52rem at ${viewport.name}px in ${theme}`, async ({ page, abwabSandbox }) => {
      ...
      const expected = Math.min(viewport.width, 52 * REM);
      expect(Math.round(box.width)).toBe(expected);

      // RTL is the app's only direction; a logical-property regression would show up as a
      // modal pinned to one side rather than centred.
      ...
      // The window is the wrong frame here: RTL puts the scrollbar on the left, so window-relative
...
test('the confirm dialog stays at 28rem with --wide in the stylesheet', async ({ page, abwabSandbox }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  const door = await abwabSandbox.createDoor({ name: abwabSandbox.uniqueName('confirm-width-probe') });
  ...
  // Task 3.13: the danger confirm's first production render. Contrast is the AA-verified pair;
  // this pins that the danger role actually reached the button rather than falling back.
...
test('a words detail modal keeps its 42rem hold-out', async ({ page }) => {
  ...
  // 42rem exceeds this viewport, so `min(100%, 42rem)` clamps — the point is that the 52rem
  // modifier did not reach it, which a 900px render proves as well as a 672px one would.
  expect(Math.round((await modal.boundingBox())!.width)).toBeLessThanOrEqual(42 * REM);
