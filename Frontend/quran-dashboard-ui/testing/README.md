...
| `npm run check:permission-catalogue` | cross-checks `AbwabPermissions.cs`, `AbwabPermissionCatalogue.cs` and the generated `core/auth/permission-codes.generated.ts`, under a count floor |
| `npm run check:audit-action-types` | both-directions parity between `AccessAuditActionType.cs` and `ACCESS_AUDIT_ACTION_TYPES`, under a count floor |
| `npm run generate:permission-codes` | rewrites `core/auth/permission-codes.generated.ts` from `AbwabPermissionCatalogue.cs`; the check above fails until its output is committed |
