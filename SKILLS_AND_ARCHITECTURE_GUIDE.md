...
- Run the pre-PR lanes `TESTING_STRATEGY.md` §5 requires for the changed scope — `tier-b` for an ordinary Backend PR, plus `pipeline --feature <key>` or `canonical-data` when the change touches `DataPipelines`, importer tools, canonical resources, pipeline tables/migrations, or shared persistence. If the change touched `Backend/api/` routes, contracts, auth, middleware, or model binding, run the affected `feature` lanes **plus** `Backend/scripts/test-backend smoke --no-build` (`TESTING_STRATEGY.md` §6); report the lane each result came from, since the canonical Smoke data tier lives in `canonical-data`, not `smoke`. There is no CI (`TESTING_STRATEGY.md` §8), so nothing runs these for you.
