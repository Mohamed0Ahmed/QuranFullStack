...
public sealed class LemmasRelatedReadTests(MorphologyExplorersTestFixture fixture)
{
    ...
    public async Task GetLemmaStems_returns_related_stems_in_deterministic_order()
    {
        ...
        var stem = response.Stems[0];
        stem.StemId.Should().Be(600);
        stem.StemText.Should().Be("كَلَّمَ");
